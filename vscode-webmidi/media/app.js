(() => {
  ...
  function renderRoutingGraph(snapshot, { live = false, preserveView = false } = {}) {
    ...
    routingSourceCopy.textContent = live ? `Live NT · ${snapshot.slots.length} slots` : "Not connected";
  ...
  async function handleRoutingConnectionClick(target) {
    ...
    if (source.element === port && source.parameterIndex != null && source.bus >= 0) {
      ...
      if (!window.confirm(`Disconnect ${name}?`)) return true;
      try {
    ...
    if (!window.confirm(`Connect ${sourceName} to ${destinationName}?`)) return true;
    try {
      if (destinationIsParameter && source.side === "output" && bus < 0) {
        ...
        await state.ntTransport.writeParameter(source.slotIndex, source.parameterIndex, bus + 1);
      }
      await state.ntTransport.writeParameter(writeTarget.slotIndex, writeTarget.parameterIndex, bus + 1);
      await loadLiveRouting({ preserveView: true });
  ...
  async function handleRoutingModeClick(target) {
    ...
    if (!window.confirm(`Change this output to ${nextName} mode?`)) return true;
    try {
  ...
  function setDeviceState(nextState) {
    ...
    else if (state.device === "labWaiting") mappingSourceCopy.textContent = "Not read yet";
  ...
  function setTransportMode(mode) {
    disconnectRealTransport();
    state.transport = mode === "real" ? "real" : "readonly";
    resetMIDIMonitor();
    $(".prototype-note").textContent = state.transport === "real" ? "Live NT · read + write" : "Live NT · read only";
  ...
  stateAction.addEventListener("click", () => {
    if (state.device === "labWaiting" || state.device === "labConnected" || state.device === "labReadOnly") {
      readRealIdentity();
      return;
    }
